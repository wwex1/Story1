/**
 * Story1 - Episode Suggestion, Response Choices, Develop Paths, Persona & Greeting Gen for SillyTavern
 * 에피소드 추천 + 전개 방향 + 선택지 생성 + 페르소나 생성 + 그리팅 생성
 */
...
const DEVELOP_PROMPT = `Based on the current roleplay context and the SELECTED episode below, generate possible development paths (story progression outlines) describing how this episode could unfold.

Each development path should:
- Break the episode into sequential beats/stages (NOT including the final ending or resolution)
- Show a natural step-by-step escalation or progression
- STOP before the climax/conclusion — leave the ending open so the story can reach it organically over multiple turns
- Stay grounded in the characters' personalities, the established world, and recent events
- Keep each beat to a single concise sentence

Generate distinctly different development paths — each should take the episode in a meaningfully different direction, not merely minor variations of the same flow.`;

// 전개 방향 출력 시 함께 붙는 영어 진행 가이드 문장 (고정)
const DEVELOP_GUIDE_EN = '- This story should progress step by step from beat 1, gradually reaching its final conclusion over multiple turns.';
...
let activeMode = 'ideas'; // 'ideas' | 'choices' | 'develop' | 'persona' | 'greeting'
let cfg = {};
let ctx = null;
...
async function mountSettings() {
    ...
    root.find('.si_enabled').prop('checked', cfg.enabled).on('change', function () {
        ...
        toastr.info(cfg.enabled ? 'Story1 활성화됨' : 'Story1 비활성화됨');
    });
    ...
    root.find('.sd_develop_enabled').prop('checked', cfg.developEnabled).on('change', function () {
        cfg.developEnabled = $(this).prop('checked'); persist();
    });
    ...
    // ─── 전개 방향 탭 ───
    root.find('.sd_count').val(cfg.developCount).on('change', function () { cfg.developCount = Number($(this).val()); persist(); });
    root.find('.sd_steps').val(cfg.developSteps).on('change', function () { cfg.developSteps = Number($(this).val()); persist(); });
    root.find('.sd_prompt').val(cfg.developPrompt).on('change', function () { cfg.developPrompt = $(this).val(); persist(); });

    root.find('.sd_prompt_reset').on('click', async function () {
        if (await ctx.Popup.show.confirm('전개 방향 프롬프트를 초기화할까요?', '설정 초기화')) {
            cfg.developPrompt = DEVELOP_PROMPT; root.find('.sd_prompt').val(DEVELOP_PROMPT); persist(); toastr.success('프롬프트 초기화됨');
        }
    });
    root.find('.sd_cache_clear').on('click', async function () {
        const total = cfg.developHistory?.length || 0;
        if (!total) { toastr.info('캐시가 없습니다.'); return; }
        if (await ctx.Popup.show.confirm(`전개 방향 캐시 ${total}건을 삭제할까요?`, '캐시 초기화')) {
            cfg.developHistory = []; cfg.developViewIdx = -1; persist();
            if (activeMode === 'develop') removeBlock(); toastr.success('전개 방향 캐시 초기화됨');
        }
    });
    ...
    root.find('.si_all_cache_clear').on('click', async function () {
        ...
        const c3 = cfg.developHistory?.length || 0;
        const c4 = cfg.personaHistory?.length || 0;
        const c5 = cfg.greetingHistory?.length || 0;
        const total = c1 + c2 + c3 + c4 + c5;
        if (!total) { toastr.info('캐시가 없습니다.'); return; }
        if (await ctx.Popup.show.confirm(`전체 캐시를 삭제할까요?\n에피소드 ${c1}건 / 선택지 ${c2}건 / 전개 ${c3}건 / 페르소나 ${c4}건 / 그리팅 ${c5}건`, '전체 캐시 초기화')) {
            cfg.cache = {}; cfg.choicesCache = {};
            cfg.developHistory = []; cfg.developViewIdx = -1;
...
async function generateDevelop(episode, isRetry) {
    if (generating || !cfg.enabled) return;
    activeMode = 'develop';
    if (cfg.apiSource === 'profile' && !cfg.connectionProfileId) { toastr.warning('Connection Profile을 선택하세요.'); return; }
    if (!ctx.chat?.length) { toastr.warning('대화 내역이 없습니다.'); return; }
    ...
    // 리롤 시: 직전 항목의 에피소드 재사용
    if (!episode && cfg.developHistory.length > 0) {
        episode = cfg.developHistory[cfg.developViewIdx]?.episode;
    }
    if (!episode || !episode.body) { toastr.warning('에피소드를 먼저 선택하세요.'); return; }

    generating = true;
    showDevelopLoading(isRetry ? '재생성 중...' : '전개 방향 생성 중...');

    try {
        const instruction = buildDevelopInstruction(episode);
        let raw = '';
        if (cfg.apiSource === 'main') {
            const bg = await gatherPlain(lastBot);
            const { generateRaw } = ctx;
        ...
        cfg.developHistory.push({ episode, directions, lang: 'ko' });
        cfg.developViewIdx = cfg.developHistory.length - 1;
        persist();
        showDevelopResult();
    } catch (err) {
        console.error(`[${EXT_NAME}]`, err); toastr.error(`전개 방향 생성 실패: ${err.message}`);
        if (cfg.developHistory.length > 0) showDevelopResult(); else removeBlock();
    } finally { generating = false; }
}

async function translateDevelop() {
    if (generating || !cfg.developHistory.length) return;
    if (cfg.apiSource === 'profile' && !cfg.connectionProfileId) { toastr.warning('Connection Profile을 선택하세요.'); return; }
    const entry = cfg.developHistory[cfg.developViewIdx];
    if (!entry) return;

    generating = true;
    showDevelopLoading('번역 중...');

    try {
        const targetLang = entry.lang === 'ko' ? 'English' : '한국어';
        const srcText = entry.directions.map((steps, di) =>
            `[${di + 1}]\n${steps.map((s, i) => `${i + 1}. ${s}`).join('\n')}`
        ).join('\n\n');
        const instruction = `Translate the following story development beats into ${targetLang}. Keep the EXACT same structure ([number] paths, numbered beats on their own lines). Output ONLY the translated content in the same format, no explanation or commentary.\n\n<directions>\n${srcText}\n</directions>`;
        let raw = '';
        if (cfg.apiSource === 'main') {
            const { generateRaw } = ctx;
            if (!generateRaw) throw new Error('generateRaw not available');
            ...
            raw = await sendProfileRequest([{ role: 'user', content: instruction }], 4000);
        }
        const directions = parseDevelop(raw);
        if (!directions?.length) throw new Error('번역 파싱 실패');
        cfg.developHistory.push({ episode: entry.episode, directions, lang: entry.lang === 'ko' ? 'en' : 'ko' });
        cfg.developViewIdx = cfg.developHistory.length - 1;
        persist();
        showDevelopResult();
    } catch (err) {
...
function showDevelopLoading(msg) {
    removeBlock();
    const block = $('<div id="si-block" class="si-block"></div>');
    block.html(`<div class="si-block-head"><span class="si-block-title">🎬 전개 방향</span></div>
...
function showDevelopResult() {
    removeBlock();
    if (!cfg.developHistory.length) return;

    const total = cfg.developHistory.length;
    const idx = cfg.developViewIdx;
    const entry = cfg.developHistory[idx];
    const episode = entry.episode || {};
    const directions = entry.directions || [];
    const guide = DEVELOP_GUIDE_EN;

    const block = $('<div id="si-block" class="si-block"></div>');
    const head = $('<div class="si-block-head"></div>');
    head.append('<span class="si-block-title">🎬 전개 방향</span>');
    ...
    prevBtn.on('click', () => { if (cfg.developViewIdx > 0) { cfg.developViewIdx--; persist(); showDevelopResult(); } });
    nextBtn.on('click', () => { if (cfg.developViewIdx < cfg.developHistory.length - 1) { cfg.developViewIdx++; persist(); showDevelopResult(); } });
    nav.append(prevBtn, navLabel, nextBtn);
    right.append(nav);
    right.append('<button class="si-block-btn dv-do-translate" title="번역">🌐</button>');
    right.append('<button class="si-block-btn dv-do-refresh" title="재생성(리롤)">🔄</button>');
    right.append('<button class="si-block-btn dv-do-delete" title="전체 삭제">🗑️</button>');
    right.append('<button class="si-block-btn dv-do-close" title="닫기">✕</button>');
    head.append(right);
    block.append(head);

    // 선택된 에피소드 표시
    const epTitle = episode.title || '선택한 에피소드';
    const epBody = episode.body || '';
    block.append(`<div class="dv-episode"><span class="dv-episode-label">📌 ${esc(epTitle)}</span>${epBody ? `<div class="dv-episode-body">${esc(epBody)}</div>` : ''}</div>`);

    const cards = $('<div class="si-cards"></div>');
    directions.forEach((steps, di) => {
        const stepsText = steps.map((s, i) => `${i + 1}. ${s}`).join('\n');
        const fullText = `${stepsText}\n${guide}`;
        const stepsHtml = steps.map((s, i) => `<div class="dv-step"><span class="dv-step-num">${i + 1}</span><span class="dv-step-text">${esc(s)}</span></div>`).join('');
        const card = $(`<div class="si-idea">
            <div class="si-idea-head"><span class="si-idea-num">${di + 1}</span><span class="si-idea-title">전개 방향 ${di + 1}</span></div>
            <div class="dv-steps">${stepsHtml}</div>
            <div class="dv-guide">${esc(guide)}</div>
            <div class="si-idea-actions">
                <button class="si-idea-act dv-act-copy">📋 복사</button>
                <button class="si-idea-act dv-act-insert">✏️ 삽입</button>
            </div>
        </div>`);
        card.find('.dv-act-copy').on('click', async () => { const ok = await copyToClipboard(fullText); if (ok) toastr.success('복사됨'); });
        card.find('.dv-act-insert').on('click', () => {
            const textarea = $('#send_textarea'); if (!textarea.length) { toastr.warning('입력창을 찾을 수 없습니다.'); return; }
            const cur = textarea.val(); textarea.val(cur ? cur + '\n' + fullText : fullText); textarea.trigger('input'); textarea.focus(); toastr.success('입력창에 삽입됨');
        });
        cards.append(card);
    });
    block.append(cards);

    block.find('.dv-do-translate').on('click', () => { if (!generating) translateDevelop(); });
    block.find('.dv-do-refresh').on('click', () => { if (!generating) generateDevelop(null, true); });
    block.find('.dv-do-close').on('click', removeBlock);
    block.find('.dv-do-delete').on('click', async () => {
        if (await ctx.Popup.show.confirm(`전개 방향 캐시 ${total}건을 삭제할까요?`, '전체 삭제')) {
            cfg.developHistory = []; cfg.developViewIdx = -1; persist(); removeBlock(); toastr.success('전체 삭제됨');
        }
    });
...
function buildDevelopInstruction(episode) {
    const epText = episode.title ? `[${episode.title}]\n${episode.body}` : episode.body;
    return `${ctx.substituteParams(cfg.developPrompt)}

=== SELECTED EPISODE ===
${epText}

⚠️ Write all development beats in Korean (한국어).

OUTPUT FORMAT - Use this EXACT structure:
<directions>
[1]
1. (first beat in Korean)
2. (second beat in Korean)
3. (third beat in Korean)
...
Rules:
- Exactly ${cfg.developCount} development paths
- About ${cfg.developSteps} beats per path
- Each path starts with [number]
- Each beat is on its own line, numbered (1. 2. 3. ...)
- Do NOT include the final ending/conclusion — stop before the climax
- Wrap in <directions>...</directions>
...
function parseDevelop(content) {
    if (!content) return null;
    const tagMatch = content.match(/<directions>\s*([\s\S]*?)\s*<\/directions>/i);
    ...
    while ((m = re.exec(body)) !== null) {
        const steps = parseSteps(m[2].trim());
        if (steps.length) directions.push(steps);
    }
    if (directions.length) return directions.slice(0, cfg.developCount || 2);

    // 폴백: [number] 구분 없이 번호 목록만 온 경우 → 단일 방향
...
function renderBlock(mode) {
    ...
    items.forEach((item, i) => {
        ...
        const showDevelop = !isChoices && cfg.developEnabled;
        const developBtn = showDevelop ? '<button class="si-idea-act si-act-develop" title="이 에피소드 전개 방향 뽑기">🎬 전개</button>' : '';
        const card = $(`<div class="si-idea"><div class="si-idea-head"><span class="si-idea-num">${i + 1}</span><span class="si-idea-title">${esc(titleText)}</span></div><div class="si-idea-desc">${esc(bodyText).replace(/\n/g, '<br>')}</div><div class="si-idea-actions"><button class="si-idea-act si-act-copy" title="복사">📋 복사</button><button class="si-idea-act si-act-insert" title="입력창에 삽입">✏️ 삽입</button>${developBtn}</div></div>`);
        ...
        if (showDevelop) {
            card.find('.si-act-develop').on('click', () => {
                if (generating) return;
                generateDevelop({ title: (item.title || `아이디어 ${i + 1}`), body: bodyText });
            });
        }
        cards.append(card);
    });
